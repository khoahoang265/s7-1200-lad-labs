...
Input values are seeded from `Watch table_1` in the project (`T1_INPUT = 5`,
`T2_INPUT = 3`, `START = TRUE`).

| # | Test case | Input | Expected | Actual | Result |
|---|---|---|---|---|---|
| 1 | Setpoints reach the timers | `T1_INPUT = 5`, `T2_INPUT = 3` | `T1_INPUT_RESULT = 5000`, `T2_INPUT_RESULT = 3000` | > ⚠️ TODO | > ⚠️ TODO |
| 2 | Start latches | `START = TRUE`, then FALSE | `LAMP = 1` and stays 1 | > ⚠️ TODO | > ⚠️ TODO |
| 3 | ON phase duration | after test 2 | `LAMP` drops after 5 s, `LAMP_1 = 1` | > ⚠️ TODO | > ⚠️ TODO |
| 4 | OFF phase duration | after test 3 | `LAMP` rises again after 3 s | > ⚠️ TODO | > ⚠️ TODO |
| 5 | Elapsed display | mid ON phase | `T1_HMI` counts 0→5 in seconds | > ⚠️ TODO | > ⚠️ TODO |
| 6 | Setpoint change on the fly | `T1_INPUT = 2` while running | next ON phase lasts 2 s | > ⚠️ TODO | > ⚠️ TODO |
| 7 | Stop | `STOP = TRUE` | `LAMP = 0`, `LAMP_1 = 0`, both timers idle | > ⚠️ TODO | > ⚠️ TODO |
| 8 | Zero setpoint | `T1_INPUT = 0` | lamp toggles every scan — document the behaviour | > ⚠️ TODO | > ⚠️ TODO |

## Demo

[![Lab 1 demo](img/hmi-root-screen.png)](https://example.com/TODO-lab1-video)
